    ...
    if request.method =='POST':
        ...
        if request.POST['mode'] == '__friends_form__':
            ...
            for item in fds:
                ...
                vlist.append(item.user.username)
            # メッセージを設定
            messages.success(request, 'チェックされたFriendを'+sel_group+\
                    'に登録しました。')
            # フォームの用意
...
def good(reqest, good_id):
    ...
    is_good = Good.objects.filter(owner=reqest.user)\
                .filter(message=good_msg).count()
    # ゼロより大きければgood済み
    if is_good > 0:
        messages.success(reqest, '既にメッセージにはgoodしています。')
        return redirect(to='/sns')
    ...
    # Goodを作成し、設定して保存
    good = Good()
    good.owner = reqest.user
    good.message = good_msg
    good.save()
    # メッセージを設定
    messages.success(reqest, 'メッセージにGoodしました！')
    return redirect(to='/sns')
